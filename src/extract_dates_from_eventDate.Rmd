...
```{r}
dates[which(str_extract_all(dates$SIR_end_year, "-") == TRUE)]
```
...
From second column `SIR_end_year`:
...
Retrieve very first date present in the three new columns:

```{r first_year_final}
df_dates <- df_dates %>%
  mutate(first_year_final = 
           if_else(!is.na(first_year_start_year),
                   first_year_start_year,
                   if_else(!is.na(second_year_start_year),
                           second_year_start_year,
                           if_else(!is.na(first_year_end_year),
                                   first_year_end_year,
                                   if_else(!is.na(first_year_end_year),
                                           first_year_end_year,
                                           NA_integer_)))))
